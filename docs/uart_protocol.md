...
| 相机引脚 | 方向 | 小车引脚 |
|---------|------|---------|
| A19 (UART1_TX) | 相机 → 小车 | RX |
| A18 (UART1_RX) | 小车 → 相机 | TX |
| GND | — | GND |
...
|------|---------|-------------|---------|
| START | `0x01` | `AA 55 01 01 01` | 目标已锁定，小车可以出发 |
...
识别到路口并投票出结果后发送，每个路口只发一次。

| 字段 | 偏移 | 类型 | 字节数 | 说明 |
|------|------|------|--------|------|
| direction | 0 | uint8 | 1 | 1 = LEFT（左转），2 = RIGHT（右转） |
| target_number | 1 | uint8 | 1 | 目标房号 1~8 |
| intersection_index | 2 | uint8 | 1 | 当前第几个路口，从 0 开始 |
| confidence | 3 | uint8 | 1 | 方向置信度 0~100 |

**帧长：8 字节**

```
AA 55  04  83  <dir:1> <tgt:1> <idx:1> <conf:1>  <cs>
```

例：1 号目标在第 0 个路口左转，置信度 100
...
| 3 | FOLLOW_LINE | 循线行驶中 |
| 4 | DECIDE_DIRECTION | 检测到路口，正在判断方向 |
...
小车 (MCU)                             相机 (MaixCam)
    ...
    │── START ─────────────────────────────→│  小车就绪，发车
    │                                       │
    │←── STATUS (3: FOLLOW_LINE) ───────────│  开始循线
    │←── LINE_DATA ... ─────────────────────│  每 2 帧发一次巡线数据
    │←── LINE_DATA ...                      小车用 error/angle 做 PID
    │←── LINE_DATA ...                      持续跟踪黑线
    │                                       │
    │←── STATUS (4: DECIDE_DIRECTION) ──────│  检测到十字路口
    │←── LINE_DATA ... ─────────────────────│  仍然继续发巡线数据
    │←── TURN_DECISION (LEFT, 房号3, 路口0) │  决策完成：左转
    ...
    │── TURN_DONE ─────────────────────────→│  转弯完成
    │                                       │
    │←── STATUS (3: FOLLOW_LINE) ───────────│  继续循线到下一个路口
    │←── LINE_DATA ...                       │
    │    ...循环直到到达目标...                │
```
...
2. **DECIDE_DIRECTION 期间 LINE_DATA 照发**。这是正常行为——决策路口方向的几帧时间里，巡线数据不中断，小车照常循线，不会跑偏。

3. **VISION_HOLD 的处理**。收到此消息说明相机不确定，小车应减速/停下等待，不要继续冲。等相机恢复自信后会重新发 LINE_DATA。

4. **TURN_DONE 后不要立刻依赖 LINE_DATA**。转弯完成后的几帧相机可能在适应新路线，line_data 短暂不稳定是正常的。

5. **发送格式**。串口助手调试时务选 HEX（十六进制）发送，不要选 ASCII。选中 ASCII 会把字符的 ASCII 码发出去，相机完全不解。

6. **校验和必须算对**。校验和不通过，整帧静默丢弃，不会有任何错误提示。调试时如果"发了没反应"，先把原始字节打出来手动核对。
